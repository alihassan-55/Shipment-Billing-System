...
export const useDataStore = create((set, get) => ({
  ...
  ledgerLoading: false,
  ledgerCustomer: null,
  ledgerSummary: null, // Stores aggregated totals (debit, credit, balance)
  ...
  fetchLedgerEntries: async (customerId = null, params = {}) => {
    set({ ledgerLoading: true })
    try {
      const url = customerId ? `/ledger/customer/${customerId}` : '/ledger'
      // Merge params, ensure we don't duplicate customerId if it's already in url path
      ...
      const pagination = response.data.pagination || { page: 1, limit: 20, total: entries.length, pages: 1 }
      ...
      set({ ledgerEntries: [], ledgerCustomer: null, ledgerPagination: { page: 1, limit: 20, total: 0, pages: 0 }, ledgerSummary: null })
    }
  },
